...
async function run() {
  ...
  const result = await sdk.manifests.list(page, results, shippoApiVersion);
...
async function run() {
  ...
  const result = await sdk.manifests.create(shippoApiVersion, manifestCreateRequest);
...
async function run() {
  ...
  const result = await sdk.manifests.get(manifestId, shippoApiVersion);
