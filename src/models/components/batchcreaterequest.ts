...
export function batchCreateRequestFromJSON(
  jsonString: string,
): SafeParseResult<BatchCreateRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => BatchCreateRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'BatchCreateRequest' from JSON`,
  );
}
