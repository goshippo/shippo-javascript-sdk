...
export function parcelCreateRequestFromJSON(
  jsonString: string,
): SafeParseResult<ParcelCreateRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ParcelCreateRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ParcelCreateRequest' from JSON`,
  );
}
