...
export const WebhookPayload$inboundSchema: z.ZodType<WebhookPayload, z.ZodTypeDef, unknown> =
    z.union([
        WebhookPayloadTrack$inboundSchema.and(
            z.object({ event: z.literal("track_updated") }).transform((v) => ({ event: v.event }))
        ),
        WebhookPayloadBatch$inboundSchema.and(
            z.object({ event: z.literal("batch_created") }).transform((v) => ({ event: v.event }))
        ),
        WebhookPayloadBatch$inboundSchema.and(
            z.object({ event: z.literal("batch_purchased") }).transform((v) => ({ event: v.event }))
        ),
        WebhookPayloadTransaction$inboundSchema.and(
            z
                .object({ event: z.literal("transaction_created") })
                .transform((v) => ({ event: v.event }))
        ),
        WebhookPayloadTransaction$inboundSchema.and(
            z
                .object({ event: z.literal("transaction_updated") })
                .transform((v) => ({ event: v.event }))
        ),
    ]);
...
> = z.union([
    ...
    WebhookPayloadTransaction$outboundSchema.and(
        z.object({ event: z.literal("transaction_created") }).transform((v) => ({ event: v.event }))
    ),
    WebhookPayloadTransaction$outboundSchema.and(
        z.object({ event: z.literal("transaction_updated") }).transform((v) => ({ event: v.event }))
    ),
]);
